...
export function PaginationWithLinks({
  ...
  pageSearchParam,
  callback
}: PaginationWithLinksProps & { callback: (page: number) => void }) {
    ...
    if (totalPageCount <= maxVisiblePages) {
        ...
        items.push(
          <PaginationItem key={i}>
            <PaginationLink
              onClick={() => callback(i)}
              isActive={page === i}
              className={page === i ? "border border-gray-200" : ""}
            >
              {i}
            </PaginationLink>
          </PaginationItem>
      ...
      items.push(
        <PaginationItem key={1}>
          <PaginationLink
            onClick={() => callback(1)}
            isActive={page === 1}
            className={page === 1 ? "border border-gray-200" : ""}
          >
            1
          </PaginationLink>
        </PaginationItem>
      ...
      items.push(
        <PaginationItem key={totalPageCount}>
          <PaginationLink
            onClick={() => callback(totalPageCount)}
            isActive={page === totalPageCount}
            className={page === totalPageCount ? "border border-gray-200" : ""}
          >
            {totalPageCount}
          </PaginationLink>
        </PaginationItem>
