...
export const generateInvoicePreviewPDF = (invoice: InvoiceHistoryType) => {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(invoice.date || formatDate(new Date(), "dd.MM.yyyy HH:mm"), margin + 20, detailsStartY);

  doc.setFont("helvetica", "bold");
  doc.text("Due Date:", margin + 3, detailsStartY + lineHeight);
  doc.setFont("helvetica", "normal");
  doc.text(formatDate(invoice.dueDate || new Date(), "dd.MM.yyyy HH:mm"), margin + 20, detailsStartY + lineHeight);
  ...
  doc.setFont("helvetica", "normal");
  doc.text(invoice.customer.name, margin + 3, customerStartY + lineHeight);
  ...
  if (invoice.items && invoice.items.length > 0) {
    tableRows = invoice.items.map((item) => [
      formatDate(item.date || new Date(), "dd.MM.yyyy HH:mm"),
  ...
  const subtotal = invoice.items && invoice.items.length > 0 ? calculateSubtotal(invoice.items) : 2.37;

  const tax = invoice.items && invoice.items.length > 0 ? calculateTaxTotal(invoice.items) : 0.38;

  const total = invoice.items && invoice.items.length > 0 ? calculateGrandTotal(invoice.items) : 2.75;
