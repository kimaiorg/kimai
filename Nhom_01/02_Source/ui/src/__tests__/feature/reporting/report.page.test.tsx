...
describe("Reporting Page", () => {
  ...
  beforeEach(() => {
    ...
    (useAppSelector as any as jest.Mock).mockImplementation((selector) => {
    ...
    jest.mocked(useAppSelector).mockImplementation((selectorFn) =>
      selectorFn({
        userState: mockUserStateObj,
        userListState: { users: mockUserList }
      })
    );
  ...
  it("Render week user reporting ui", async () => {
    ...
    await waitFor(() => {
      ...
      const expectedHeaders = ["Task", "Total", "MON 2", "TUE 3", "WED 4", "THU 5", "FRI 6", "SAT 7", "SUN 8"];
  ...
  it("Render project review report ui", async () => {
    ...
    const thead = rowgroups.find((el) => el.tagName.toLowerCase() === "thead")!;
    const headers = within(within(thead).getByRole("row")).getAllByRole("columnheader");
    const expectedHeaders = ["Name", "Hourly Quota", "Budget", "Last Entry", "This Month", "Total", "Not Exported"];
